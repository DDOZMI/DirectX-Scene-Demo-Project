...
LightShaderClass::LightShaderClass()
{
	...
	m_brightnessBuffer = nullptr;

	m_currentFilterMode = 0;
}
...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount,
	...
	XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower, float time, float modelID)
{
	bool result;

	// 렌더링에 사용될 쉐이더 파라미터 설정하기
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture,
		lightDirection, ambientColor, diffuseColor, cameraPosition, specularColor, specularPower, time, modelID);
	if (!result)
	{
		return false;
	}
	...
	{
		// 다른 모델들은 일반 렌더링
		//deviceContext->DrawIndexed(indexCount, 0, 0); //이렇게 사용해도 가능함
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* fileName)
{
	...
	if (!CreateSamplerState(device, 1))
	{
		return false;
	}
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext,
	...
	float specularPower, float time, float modelID)
{
	HRESULT result;
...
bool LightShaderClass::SetBrightnessParameter(ID3D11DeviceContext* deviceContext, float brightness)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	BrightnessBufferType* dataPtr;
	...
	dataPtr = (BrightnessBufferType*)mappedResource.pData;
	dataPtr->brightness = brightness;

	deviceContext->Unmap(m_brightnessBuffer, 0);

	deviceContext->PSSetConstantBuffers(0, 1, &m_brightnessBuffer);

	return true;
}

// 여기서 샘플러 설정 전부 관리
bool LightShaderClass::CreateSamplerState(ID3D11Device* device, int filterMode)
{
	HRESULT result;
	D3D11_SAMPLER_DESC samplerDesc;
	...
	samplerDesc.MaxAnisotropy = 16;  // 비등방성 필터링 레벨 설정
...
bool LightShaderClass::SetTextureFilter(ID3D11Device* device, int filterMode)
{
	// 현재 필터와 같으면 변경X
...
bool LightShaderClass::SetupShaderResources(ID3D11DeviceContext* deviceContext,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture,
	XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor,
	float specularPower, float time, float modelID)
