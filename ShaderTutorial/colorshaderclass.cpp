...
bool ColorShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float brightness, float time)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, time);
	if(!result)
	{
		return false;
	}

	result = SetBrightnessParameter(deviceContext, brightness);
	if (!result)
	{
		return false;
	}
...
bool ColorShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float time)
{
	HRESULT result;
...
bool ColorShaderClass::SetBrightnessParameter(ID3D11DeviceContext* deviceContext, float brightness)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
